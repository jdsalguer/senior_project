/****************************************************************************
** Meta object code from reading C++ file 'ocularinstruc.h'
**
** Created: Sat Mar 28 18:34:07 2015
**      by: The Qt Meta Object Compiler version 62 (Qt 4.6.3)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../projectPHOE/ocularinstruc.h"
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'ocularinstruc.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 62
#error "This file was generated using the moc from 4.6.3. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
static const uint qt_meta_data_ocularinstruc[] = {

 // content:
       4,       // revision
       0,       // classname
       0,    0, // classinfo
       2,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       0,       // signalCount

 // slots: signature, parameters, type, tag, flags
      15,   14,   14,   14, 0x08,
      46,   14,   14,   14, 0x08,

       0        // eod
};

static const char qt_meta_stringdata_ocularinstruc[] = {
    "ocularinstruc\0\0on_ocuinstbackButton_clicked()\0"
    "on_ocuinstbegButton_clicked()\0"
};

const QMetaObject ocularinstruc::staticMetaObject = {
    { &QDialog::staticMetaObject, qt_meta_stringdata_ocularinstruc,
      qt_meta_data_ocularinstruc, 0 }
};

#ifdef Q_NO_DATA_RELOCATION
const QMetaObject &ocularinstruc::getStaticMetaObject() { return staticMetaObject; }
#endif //Q_NO_DATA_RELOCATION

const QMetaObject *ocularinstruc::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->metaObject : &staticMetaObject;
}

void *ocularinstruc::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_ocularinstruc))
        return static_cast<void*>(const_cast< ocularinstruc*>(this));
    return QDialog::qt_metacast(_clname);
}

int ocularinstruc::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QDialog::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        switch (_id) {
        case 0: on_ocuinstbackButton_clicked(); break;
        case 1: on_ocuinstbegButton_clicked(); break;
        default: ;
        }
        _id -= 2;
    }
    return _id;
}
QT_END_MOC_NAMESPACE
